class TestClubsArray:
    def test_get_clubs_array_returns_200_with_valid_clubs(self, client):
    ...
    def test_clubs_array_displays_the_correct_information(self, client, clubs):
        response = client.get("/clubsArray")
        response = response.data.decode()
        for club in clubs:
            assert (
    ...
    def test_clubs_array_returns_the_correct_template(self, client):
        response = client.get("/clubsArray")
        response = response.data.decode()
